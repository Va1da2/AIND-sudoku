...
# Function to make box names (from lesson)
def cross(A, B):
...
# Lists and dicts to help traverse sudoku grid
row_units = [cross(r, cols) for r in rows]
column_units = [cross(rows, c) for c in cols]
square_units = [cross(rs, cs) for rs in ('ABC','DEF','GHI') for cs in ('123','456','789')]
# Making diagonal units to be able to solve diagonal sudoku
diagonal1 = [rows[i]+cols[i] for i in range(9)]
diagonal2 = [rows[-i-1]+cols[i] for i in range(9)]
# Adding all units to the same list
...
def naked_twins(values):
    ...
    # iterate through unit lists 
    for l in unitlist:
        twins = {}
        # Get values for the boxes
        unit_values = {k: v for k, v in values.items() if k in l}
        # Search boxes with two available values
        ...
        if twins:
            for d in twins:
                # Take twin boxes
                tw1, tw2 = twins[d]
                # Get intersection of two sets to get common peers
                intersect = peers[tw1].intersection(peers[tw2])
                # Remove twins from peers 
                for b in intersect:
                    for dd in d:
                        value = values[b].replace(dd, '')
                        values = assign_value(values, b, value)
